#!/bin/bash
# verify.sh — compile Math.sol with solc 0.1.0 and compare to on-chain bytecode
# Contract: 0x258c09146b7a28Dde8d3e230030e27643F91115F
...
echo "Compiling Math.sol with $IMAGE (solc 0.1.0, optimizer OFF) ..."
RAW=$(docker run --rm -v "$PWD:/src" --entrypoint /umbrella/build/solc/solc "$IMAGE" --binary stdout /src/Math.sol 2>/dev/null)
...
if [ $ok -eq 1 ]; then
    echo
    echo "Verified: 0x258c09146b7a28Dde8d3e230030e27643F91115F"
    exit 0
else
    exit 1
fi
